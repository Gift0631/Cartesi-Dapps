...
const quadraticEquation = (a, b, c) => {
  ...
  const squareRootOfRoot = Math.sqrt(root);
  const denominator = 2 * a;
  
  const firsAnswer = (-b + squareRootOfRoot)/denominator
  const secondAnswer = (-b - squareRootOfRoot)/denominator

  return `The roots of your equation is ${firsAnswer} or ${secondAnswer}`
}
...
async function handle_inspect(data) {
  ...
  const responseObject = "Inspect route not implemented for quadratic equation calculator";
